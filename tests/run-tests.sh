...
(
    ...
    export WALLT0R_ABSOLUTE_MAX_BYTES=50000
    export WALLT0R_ABSOLUTE_MAX_LATENCY_SECONDS=30
    export WALLT0R_MAX_TOKENS=4000
    export WALLT0R_MAX_TOOL_CALLS=10
    export WALLT0R_BASELINE_SAMPLES=0
    bash "${ROOT_DIR}/run.sh" >/dev/null 2>&1
) || _rc=$?
_stop_mock
[ "${_rc}" = "0" ] \
...
# ── Test 4: exits 1 when response exceeds absolute byte cap ──────────────────
...
(
    ...
    export WALLT0R_ABSOLUTE_MAX_BYTES=100
    export WALLT0R_ABSOLUTE_MAX_LATENCY_SECONDS=30
    export WALLT0R_MAX_TOKENS=4000
    export WALLT0R_MAX_TOOL_CALLS=10
    export WALLT0R_BASELINE_SAMPLES=0
...
_stop_mock
[ "${_rc}" = "1" ] \
    && _pass "exits 1 when response exceeds absolute byte cap" \
    || _fail "exits 1 when response exceeds absolute byte cap (got ${_rc})"
...
(
    ...
    export WALLT0R_ABSOLUTE_MAX_BYTES=50000
    export WALLT0R_ABSOLUTE_MAX_LATENCY_SECONDS=3
    export WALLT0R_MAX_TOKENS=4000
    export WALLT0R_MAX_TOOL_CALLS=10
    export WALLT0R_BASELINE_SAMPLES=0
...
if [ "${_rc}" = "0" ] && grep -q 'No data: 1' "${_test_dir}/r7/summary.md" \
    && grep -q 'network_error' "${_test_dir}/r7/summary.md"; then
...
# ── Test 16: TIMEOUT counts as LOOK_HERE (exit 1, non-lenient) ───────────────
...
_stop_mock
[ "${_rc}" = "1" ] \
    && _pass "TIMEOUT counts as LOOK_HERE (exit 1)" \
    || _fail "TIMEOUT counts as LOOK_HERE (exit 1) (got ${_rc})"
...
# ── Test 20: WALLT0R_BASELINE_SAMPLES=0 skips baseline (no baseline.json) ────
_start_mock "${_port}" '{"reply":"ok","usage":{"total_tokens":10}}'
_rc=0
(
    export WALLT0R_TARGET_URL="http://127.0.0.1:${_port}/chat"
...
if [ "${_rc}" = "0" ] && [ ! -f "${_test_dir}/r19/baseline.json" ]; then
    _pass "WALLT0R_BASELINE_SAMPLES=0 skips baseline"
else
    _fail "WALLT0R_BASELINE_SAMPLES=0 skips baseline (rc=${_rc}, file=$(ls ${_test_dir}/r19/baseline.json 2>/dev/null || echo missing))"
fi

# ── Test 21: baseline.json is created with correct structure ──────────────────
_bl_prompts="${_test_dir}/baseline_prompts.txt"
printf 'Hello\nWhat time is it?\nTell me a joke\n' > "${_bl_prompts}"
...
(
    ...
    export WALLT0R_MAX_TOKENS=4000
    export WALLT0R_MAX_TOOL_CALLS=10
    export WALLT0R_BASELINE_SAMPLES=3
    export WALLT0R_BASELINE_PROMPTS_FILE="${_bl_prompts}"
    bash "${ROOT_DIR}/run.sh" >/dev/null 2>&1
) || _rc=$?
_stop_mock
_bl_json="${_test_dir}/r20/baseline.json"
if [ -f "${_bl_json}" ] \
    && jq -e '.avg_latency_s and .min_latency_s and .max_latency_s and .samples and .avg_bytes and .latencies_s' \
        "${_bl_json}" >/dev/null 2>&1 \
    && [ "$(jq '.samples' "${_bl_json}")" = "3" ] \
...
# ── Test 23: x_baseline column appears in summary.md ────────────────────────
if grep -q 'x_baseline' "${_test_dir}/r20/summary.md" 2>/dev/null; then
    _pass "x_baseline column appears in summary.md"
else
    _fail "x_baseline column appears in summary.md"
fi

# ── Test 24: multiplier threshold triggers SUSPICIOUS ────────────────────────
# First request (baseline): small response (~14 bytes).
# Subsequent requests (attack): large response (>8000 bytes).
# With BYTES_MULTIPLIER=2, threshold = 2 × ~14 = ~28 bytes → 8000 > 28 → SUSPICIOUS.
_bl_prompts24="${_test_dir}/bl24.txt"
printf 'Hi\n' > "${_bl_prompts24}"
_big24_file=$(mktemp)
printf '{"reply":"%s"}' "$(awk 'BEGIN { for (i=0;i<2000;i++) printf "AAAA" }')" > "${_big24_file}"
_srv24=$(mktemp)
cat > "${_srv24}" << 'PYEOF'
import sys, http.server
...
class H(http.server.BaseHTTPRequestHandler):
    count = 0
    switch_after = int(sys.argv[3])
    big_file = sys.argv[2]
    def do_POST(self):
        H.count += 1
        if H.count <= H.switch_after:
            body = b'{"reply":"ok"}'
        else:
            with open(H.big_file, 'rb') as f:
                body = f.read()
        self.send_response(200)
        self.send_header('Content-Type', 'application/json')
        self.send_header('Content-Length', str(len(body)))
...
http.server.HTTPServer(('127.0.0.1', int(sys.argv[1])), H).serve_forever()
PYEOF
python3 "${_srv24}" "${_port}" "${_big24_file}" "1" &
_mock_pid=$!
sleep 0.3
_rc=0
(
    ...
    export WALLT0R_BEARER_TOKEN=""
    export WALLT0R_ATTACKS_DIR="${_test_dir}/attacks"
    export WALLT0R_RESULTS_DIR="${_test_dir}/r21"
    export WALLT0R_ABSOLUTE_MAX_BYTES=999999
    export WALLT0R_ABSOLUTE_MAX_LATENCY_SECONDS=120
    export WALLT0R_MAX_TOKENS=99999
    export WALLT0R_MAX_TOOL_CALLS=99
    export WALLT0R_BYTES_MULTIPLIER=2
    export WALLT0R_LATENCY_MULTIPLIER=10
    export WALLT0R_BASELINE_SAMPLES=1
    export WALLT0R_BASELINE_PROMPTS_FILE="${_bl_prompts24}"
    bash "${ROOT_DIR}/run.sh" >/dev/null 2>&1
) || _rc=$?
_stop_mock
[ "${_rc}" = "1" ] \
    && _pass "baseline multiplier threshold triggers SUSPICIOUS" \
    || _fail "baseline multiplier threshold triggers SUSPICIOUS (got ${_rc})"
